...
static DWORD* const pResWidth = (DWORD*)0x8F436C;
static DWORD* const pResHeight = (DWORD*)0x8F4370;
...
{
    int volume = CMenuManager::m_nPrefsMusicVolume;
    if (volume < 0) volume = 0;
    if (volume > 127) volume = 127;
    return static_cast<unsigned char>(volume);
}
int gCurrentStation = -1;

// VC's "Radio Volume" slider (m_nPrefsMusicVolume) ranges 0..127.
...
static bool gSfxLoadAttempted = false;
static bool gInitialStationQueued = false;
...
{
    gSfxLoadAttempted = true;

    std::string baseFolder = gGameFolder;
    std::string sdtPath = baseFolder + "audio\\SFX.SDT";
    std::string rawPath = baseFolder + "audio\\SFX.RAW";

    // If the ASI is installed directly in the GTA root instead of a scripts
    // subfolder, gGameFolder may already be the module folder's parent. Fall back
    // to the module folder so root\audio installs do not spam missing-SFX logs.
    if (!FileExists(sdtPath) || !FileExists(rawPath)) {
        std::string moduleSdt = gScriptsFolder + "audio\\SFX.SDT";
        std::string moduleRaw = gScriptsFolder + "audio\\SFX.RAW";
        if (FileExists(moduleSdt) && FileExists(moduleRaw)) {
            baseFolder = gScriptsFolder;
            sdtPath = moduleSdt;
            rawPath = moduleRaw;
        }
    }
    ...
    if (!sdt.is_open()) {
        gLog << "SFX: cannot open " << sdtPath << std::endl;
        gLog.flush();
        return;
    }
    std::ifstream raw(rawPath, std::ios::binary);
    if (!raw.is_open()) {
        gLog << "SFX: cannot open " << rawPath << std::endl;
        gLog.flush();
        return;
    }

    gLog << "SFX: loading from " << baseFolder << "audio\\" << std::endl;
...
static void PlayTuningSound()
{
    ...
    if (gSfxTuningStream) {
        BASS_ChannelSetAttribute(gSfxTuningStream, BASS_ATTRIB_VOL, RadioVolume(GetMusicVolume()));
...
static void StartStaticSound()
{
    ...
    if (gSfxStaticStream) {
        BASS_ChannelSetAttribute(gSfxStaticStream, BASS_ATTRIB_VOL, RadioVolume(GetMusicVolume()));
...
static void ForceNativeRadioOff(CVehicle* pVehicle)
{
    bRadioOff = true;
    gMusicManager.m_nRadioInCar = 10;
    gMusicManager.m_nRadioStation = 10;
    gMusicManager.m_bRadioSetByScript = false;
    gMusicManager.m_bDoTrackService = false;

    // GTA III's cMusicManager::ServiceTrack starts the native radio on streamed
    // channel 0 when m_bDoTrackService is set. Stop that channel while the player
    // is in a vehicle so the game's default radio cannot play underneath BASS.
    plugin::CallMethod<0x5680E0, cSampleManager*, unsigned char>(&SampleManager, 0);

    if (pVehicle)
        pVehicle->m_nRadioStation = 10;
}
...
{
    unsigned char vol = GetMusicVolume();
...
static bool GenerateDefaultINI(const std::string& iniPath)
{
    std::ofstream out(iniPath, std::ios::out | std::ios::trunc);
    if (!out.is_open())
        return false;

    out << "# III.AmbientRadio autogenerated default configuration.\n";
    out << "# This file is created only when missing; existing configs are never overwritten.\n";
    out << "# Station paths are relative to the GTA III game folder. Replace them with your ported radio files as needed.\n\n";
    ...
    out << "RadioIconY=0.05\n";
    out << "ScriptIntegration=1\n\n";

    out << "[CONTROLS]\n";
    out << "# Virtual-key code; 82 is R. Hold for ~2.5 seconds to turn radio off.\n";
    out << "RadioSwitchNext=82\n";
    out << "# XInput button mask, 0 disables controller radio switching.\n";
    out << "RadioSwitchNextPad=0\n\n";

    out << "[STATIONS]\n";
    out << "HEAD RADIO | audio\\HEAD.wav\n";
    out << "DOUBLE CLEF FM | audio\\CLASS.wav\n";
    out << "K-JAH | audio\\KJAH.wav\n";
    out << "RISE FM | audio\\RISE.wav\n";
    out << "LIPS 106 | audio\\LIPS.wav\n";
    out << "GAME RADIO | audio\\GAME.wav\n";
    out << "MSX FM | audio\\MSX.wav\n";
    out << "FLASHBACK FM | audio\\FLASH.wav\n";
    out << "CHATTERBOX FM | audio\\CHAT.wav\n\n";

    out << "[VEHICLES]\n";
    out << "# Optional per-model station mapping: modelId | Station Name\n";
    out << "# 90 | HEAD RADIO\n\n";

    out << "[NORADIO]\n";
    out << "# Optional vehicle model IDs that should never play radio. One ID per line.\n\n";

    out << "[STARTOFFSET]\n";
    out << "# Optional station start offsets: Station Name | M:SS.mmm\n";
    out << "# HEAD RADIO | 0:00\n\n";

    out << "[STATIONICONS]\n";
    out << "# Optional icon mapping: Station Name | textureName (loads RadioHud\\textureName.png)\n";
    out << "# HEAD RADIO | head\n";
...
static void LoadINI()
{
    ...
    std::string parentFolder = noSlash.substr(0, noSlash.find_last_of("\\/") + 1);
    gGameFolder = FileExists(gScriptsFolder + "audio\\SFX.SDT") ? gScriptsFolder : parentFolder;

    std::string iniPath = gScriptsFolder + "III.AmbientRadio.ini";
    std::string logPath = gScriptsFolder + "III.AmbientRadio.log";
    ...
    if (!iniBin.is_open()) {
        if (GenerateDefaultINI(iniPath)) {
            gLog << "INI: III.AmbientRadio.ini not found; generated default config" << std::endl;
            iniBin.open(iniPath, std::ios::binary);
        }
        else {
            gLog << "ERROR: III.AmbientRadio.ini not found and default config could not be generated" << std::endl;
            gLog.flush();
            return;
        }
    }
    if (!iniBin.is_open()) {
        gLog << "ERROR: III.AmbientRadio.ini generated but could not be reopened" << std::endl;
        gLog.flush();
        return;
    }
    ...
    if (rawBytes.size() >= 2 && rawBytes[0] == 0xFF && rawBytes[1] == 0xFE) {
        ...
        gLog << "ERROR: III.AmbientRadio.ini is saved as RTF format." << std::endl;
...
static bool EnsureBassReady()
{
    if (!gBassReady) {
        if (BASS_Init(-1, 44100, 0, 0, NULL)) {
            ...
            if (bassError == BASS_ERROR_ALREADY) {
                gBassReady = true;
            }
            else {
                static int sLastBassInitError = 0;
                if (bassError != sLastBassInitError) {
                    gLog << "BASS init error: " << bassError << std::endl;
                    sLastBassInitError = bassError;
                }
            }
        }

        if (gBassReady) {
            gLog << "BASS ready: 1" << std::endl;
            gLog.flush();
        }
    }
    ...
    if (!gSfxLoaded && !gSfxLoadAttempted)
        LoadAllSfx();

    if (!gInitialStationQueued && !stations.empty()) {
        StartLoadingStation(0);
        gInitialStationQueued = true;
    }
        ...
        Events::gameProcessEvent.Add([]()
            {
                ...
                // BASS can fail during early game init if the device is not ready or
                // another plugin races us. Retry from the game loop until it is ready;
                // without this, one transient init failure leaves all radio streams silent.
                EnsureBassReady();
                ...
                if (inVehicle && pVehicle) {
                    ...
                    // Force the native radio state back to off every frame after reading.
                ...
                bool isSeated = (pPlayer->m_pVehicle != nullptr);
        ...
        Events::drawHudEvent.Add([]()
            {
                ...
                if (gPlayerInVehicle && gActiveVehicle)
                    ForceNativeRadioOff(gActiveVehicle);
